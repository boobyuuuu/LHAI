    ...
    def __init__(self, num_to_learn, path_data, inverse=False, data_range=1.0):
        ...
        for i in range(idx_beg,idx_end):
            ...
            if abs(data_range - 1.0) < 1e-5:
                img_blurry = (blurry_data - blurry_data.min()) / (blurry_data.max() - blurry_data.min() + 1e-8)
                img_original = (original_data - original_data.min()) / (original_data.max() - original_data.min() + 1e-8)

            elif abs(data_range - 2.0) < 1e-5:
                img_blurry = 2 * (blurry_data - blurry_data.min()) / (blurry_data.max() - blurry_data.min() + 1e-8) - 1
                img_original = 2 * (original_data - original_data.min()) / (original_data.max() - original_data.min() + 1e-8) - 1
...
class DataModule:
    ...
    def build(self):
        ...
        self.dataset = ImageDataset(filelen, self.data_path, inverse=self.inverse, data_range = self.data_range)
