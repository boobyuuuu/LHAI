...
int main(int argc, char *argv[]){
    ...
    if (argc!=5){
        ...
        cout<<argv[0]<<"\n  [ configfile : Fit.yaml ]\n  [ configfile : ParInit.yaml ]\n  [ Poisson flag : 0/1 ]\n  [ Random seed ]"<<endl;
        return -1; 
    }

    int poisson_flag = 1;
    sscanf(argv[3], "%d", &poisson_flag);
    cout<<" *** main : Poisson flag: "<<poisson_flag<<endl;
    UInt_t poisson_seed;
    sscanf(argv[4], "%u", &poisson_seed);
    cout<<" *** main : Poisson seed: "<<poisson_seed<<endl;
    ...
    if (FitFlag>0){
        ...
        // Cal expected excess
        double *par_dge = new double[npar_total];
        for (int ii=0;ii<npar_total;ii++)
            par_dge[ii] = 0;
        int npar_temp = 0;
        for (int isrc=0;isrc<WCDAFit->Template->NTemp;isrc++){
            // SED
            for (int ipar=0;ipar<WCDAFit->Template->DGEs[isrc].nSEDpar;ipar++)
                par_dge[npar_temp+ipar] = WCDAFit->Template->DGEs[isrc].SEDPar[ipar][0];
            npar_temp += WCDAFit->Template->DGEs[isrc].nSEDpar;
        }

        if (cf.UseWCDA)
            WCDAFit->Convolute_DGE(par_dge, 0, cf.NnhitUsed, -1, par_dge, par_dge);
        if (cf.UseKM2A)
            KM2AFit->Convolute_DGE(par_dge, 0, cf.KNEbinUsed, -1, par_dge, par_dge);

        // Output
        if (cf.fOut[1]!="none"){
            FitMode->MkOutdir();
            FitMode->OutConvExcess_ai(Template, ROI->Cellid, WCDAFit->Nmodel_convo, KM2AFit->Nmodel_convo, WCDAFit->WCDAData->Nbkg, KM2AFit->KM2AData->Nbkg, poisson_flag, poisson_seed);
        }
