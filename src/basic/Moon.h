...
void moon_orbit( double mjd, double *MRAS, double *MDEC ){
  ...
  m = sin( SUMP * deg_rad ) * CCC * cos( wcda_la * deg_rad ) / cos( DEC );
  ...
  *MRAS = ( atan( m * sin( RAS - theta ) / ( 1 - m * cos( RAS - theta ) ) ) + RAS ) * rad_deg;
  ...
  gamma = atan( SSS / CCC * tan( wcda_la * deg_rad ) * cos( ( *MRAS * deg_rad - RAS ) / 2.0 ) 
		/ cos( ( *MRAS * deg_rad + RAS ) / 2.0 - theta ) );
  n = sin( SUMP * deg_rad ) * SSS * sin( wcda_la * deg_rad ) / sin( gamma );
