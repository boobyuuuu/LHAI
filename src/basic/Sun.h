...
#define  CCC 1.0015174
#define  SSS 0.9948174
...
void sun_orbit( double mjd, double *SRAS, double *SDEC ){
  ...
  *SDEC = SUMD;
  ...
  m = sin( SUMP * deg_rad ) * CCC * cos( tibet_la * deg_rad ) / cos( DEC );
  ...
  *SRAS = ( atan( m * sin( RAS - theta ) / ( 1 - m * cos( RAS - theta ) ) ) + RAS ) * rad_deg;
  ...
  gamma = atan( SSS / CCC * tan( tibet_la * deg_rad ) * cos( ( *SRAS * deg_rad - RAS ) / 2.0 ) 
		/ cos( ( *SRAS * deg_rad + RAS ) / 2.0 - theta ) );
  n = sin( SUMP * deg_rad ) * SSS * sin( tibet_la * deg_rad ) / sin( gamma );
