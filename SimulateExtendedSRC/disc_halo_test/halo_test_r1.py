#python halo_test_r1.py --fitsout halo_test_r1.fits --o halo_test_r1.npy -n 100 --xsize_fin 64
import argparse
...
def Proc(filename, r1_val):
    global xsize, pw, xsize_fin, map_tree, det_res, flux_fixed, width_fixed
    np.random.seed()
    fitsfile = filename
    ...
    blur = respconv(
        fitsfile, map_tree, det_res, './halo_test_r1.hd5',  
...
flux_fixed = 1.0
width_fixed = 0.2
r1_continuous = np.linspace(0.2, 1, NIMG)

if args.idx_list is None:
    orig = []
    blur = []
    for i in tqdm.tqdm(range(NIMG)):
        current_r1 = r1_continuous[i]
        origmap, blurmap = Proc(fitsname, r1_val=current_r1)
        orig.append(origmap)
        blur.append(blurmap)
    orig = np.array(orig)
    blur = np.array(blur)
    ...
    for i in tqdm.tqdm(idx_to_process):
        current_r1 = r1_continuous[i]
        orig_new, blur_new = Proc(fitsname, r1_val=current_r1)
        orig[i] = orig_new
        blur[i] = blur_new
