#python halo_test_flux.py --fitsout halo_test_flux.fits --o halo_test_flux.npy -n 100 --xsize_fin 64
import argparse
...
def Proc(filename, flux_val):
    global xsize, pw, xsize_fin, map_tree, det_res, r1_fixed, width_fixed
    np.random.seed()
    fitsfile = filename
    ...
    blur = respconv(
        fitsfile, map_tree, det_res, './halo_test_flux.hd5',  
...
r1_fixed = 0.8
width_fixed = 0.2
flux_continuous = np.linspace(0.2, 0.8, NIMG)

orig = []
blur = []
for i in tqdm.tqdm(range(NIMG)):
    current_flux = flux_continuous[i]
    origmap, blurmap = Proc(fitsname, flux_val=current_flux)
    orig.append(origmap)
    blur.append(blurmap)
